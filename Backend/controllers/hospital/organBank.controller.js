...
export const getOrganInventory=async(req,res)=>{
    try {
        ...
        const inventory=await OrganBank.find({hospitalId:hospital._id}).sort({organType:1})
...
export const updateOrganStock=async(req,res)=>{
    try {
        ...
        const organBank=await OrganBank.findOneAndUpdate({
            hospitalId:hospital._id,organType:organType
        },{$inc:{availableCount:delta}},{new:true,upsert:true,runValidators:true})
        ...
        return res.status(200).json({success:true,message:"Stocks updated successfully",data:organBank})
